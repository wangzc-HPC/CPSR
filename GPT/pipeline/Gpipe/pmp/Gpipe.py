...
num_microbatches = 2  # Split each batch into 4 micro-batches
...
for epoch in range(epochs):
    for i, data in tqdm(enumerate(train_loader, 0)):
        ...
        if i % 50 == 0 and i != 0:
            ...
            print("50 iterations training time is {}s".format(end_50iter - train_start))

        if i % ck_interval == 0 and epoch != 0:
            ck_path = os.path.join("./checkpoint/gpt2_medium/8n8g",
            ...
            PreRecover_ck.save_checkpoint(Stage_list[global_rank].sub_model_save,
                                          ...
                                          ck_path)
            ckpt_end = time.time()
            print("rank_{} checkpoint saving time is {}s".format(global_rank, ckpt_end - ckpt_start))